...
<div class="jp_header" <?php include('module/bg_img.php'); ?>>
    <div class="container">
        <div class="row">
            <?php include('common/header_user.php'); ?>   
            <div class="col-md-12">
                <div class="jp_page_title">
                    <h3 id=""><?= $controller->set_language('chat_text'); ?></h3>
                </div>
            </div>
        </div>
    </div>
...
if(!empty($chat_info))
{
    if(isset($_SESSION['chat_system']) && $_SESSION['chat_system'] == 'true'){
                    ...
                    <div class="col-md-12">
                        <div class="jp_job_box applied_user">
                            <div class="job_detail">
                                <?php 
                                    $chat_uid = explode('-',$chat['user_from']);
                                    $user_info = $this->Common_model->select_data('name,img',$chat_uid[0],array('id' => $chat_uid[1]));	
                                ?>
                                <div class="image">
                                    <img src="<?= base_url($user_info[0]['img']) ?>" alt="">
                                </div>
                                <div class="detail">
                                    <h3 style="margin: 20px auto;"><?= $user_info[0]['name']; ?></h3>
                                </div>
                            </div>
                            <div class="jp_view_btn">
                                <div class="jp_openChatBtnWrapper" data-uid="<?= $chat_uid[1]; ?>">
                                    <span class="jp_chat_count"></span>
                                    <a class="jp_btn jp_open_chat_box" data-rid="<?= $chat_uid[1]; ?>" data-sid="<?= $_SESSION['user_id']; ?>"><?= $controller->set_language('chat_btn_text'); ?></a> 
                                </div>
                            </div>
                        </div>
                    </div>
