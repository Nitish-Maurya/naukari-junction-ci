...
<div class="hs_alert_wrapper open_alert  hs_success msg1">
    <div class="hs_alert_inner">
        <p> <span class="icon"></span>1 Designation <?= $msg ?></p>
    </div>
</div>
<?php
}
...
<div class="row" onload="location_fetch()">
    <div class="col-md-12">
        <div class="hs_heading medium">
            <h3>All Designation (<?= $this->Common_model->row_count('designation'); ?>) <a href="#add_desi_popup" class="btn" data-toggle="modal">Add new</a></h3>
        </div>
        ...
        <tbody>
                    ...
                    if(isset($row))
                    {
                     ...
                     <tr>
						...
						<td>
							<select id="desi<?= $r1->id; ?>" onchange="desi_status(<?=  $r1->id; ?>)">
								<option <?php  if($r1->status=='Active') { echo "selected"; } ?> >Active</option>
								<option <?php  if($r1->status=='Inactive') { echo "selected"; } ?> >Inactive</option>
							</select>
						</td>
                       <td width="200">
							<a  class="btn" title="Edit" data-name="Dentist" onclick="update_desi('<?= $r1->id; ?>','<?= $r1->name; ?>')"><i class="fa fa-pencil" aria-hidden="true"></i></a>
                            <a  class="btn" title="Delete" onclick="delete_conform(<?= $r1->id;  ?>,'desi')" ><i class="fa fa-trash" aria-hidden="true"></i></a>
                        </td>
                     </tr>
                     <?php
                        }
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="hs_input">
                    ...
                    <form id="f1"><input type="text" name="name" id="name1" class="form-control" placeholder="Designation Name"></form>
                </div>
				<button id="add_desi" class="btn">Add</button>
			</div>
		</div>
	</div>
</div>
...
<div id="delete_desi_popup" class="modal" role="dialog">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<button  type="button" class="close" data-dismiss="modal">&times;</button>
				<h4 class="modal-title">Do you want to delete this designation</h4>
			</div>
			<div class="modal-body">
				<?= form_open('admin/delete_designation'); ?>
				<div class="hs_input">
