                ...
                <div class="col-md-12">
					...
					<div class="jp_job_box">
                        <div class="job_detail">
                           ...
                           <span><a href="<?= base_url('recruiter/applied_user/'.$res->id); ?>"><?= $res->application." "; ?><?= $controller->set_language('r_applied'); ?></a></span>
                            <ul>
                                <li title="<?= $controller->set_language('r_tool_tip1'); ?>"><span><img src="<?= base_url('assets/images/experience.svg'); ?>" alt=""></span> <?= $res->exp; ?></li>
                                <li title="<?= $controller->set_language('r_tool_tip2'); ?>"><span><img src="<?= base_url('assets/images/location.svg'); ?>" alt=""></span> <?= $res->job_location; ?></li>
                            </ul>
                            <h4 title="<?= $controller->set_language('r_tool_tip3'); ?>">
                                <span class="icon"></span>
                                <span class="skills"><?= $res->technology; ?></span>
                            </h4>
                        </div>
						<?php
						...
						$sp2=str_replace(' ', '-', $sp);
						$location=strtolower($res->job_location);
						$location2=str_replace(' ', '-',$location);
						?>
                        <div class="jp_view_btn">
                            <a href="<?= base_url('recruiter/user_jobsingle/'.$res->id.'/'.$sp2.'-'.'jobs-in-'.$string.'-'.$location2); ?>" class="jp_btn jp_btn_block"><?= $controller->set_language('r_view'); ?></a>
							<a href="<?= base_url('recruiter/delete_job/'.$res->id); ?>" class="jp_btn jp_btn_block">Delete</a>
							 <a href="<?= base_url('recruiter/edit_job_post/'.$res->id.'/'.$sp2.'-'.'jobs-in-'.$string.'-'.$location2); ?>" class="jp_btn jp_btn_block"><?= $controller->set_language('r_edit'); ?></a>
                        </div>	
                    </div>
					<?php
					}
				   if(isset($link))
				   {
					...
					<div class="jp_job_box">
						...
						<div class="jp_no_doctor">
							...
							<p><?= $controller->set_language('job_not_found_msg'); ?></p>
						</div>
                    <!-- no job found end -->
						</div>
                    <!-- no job found end -->
