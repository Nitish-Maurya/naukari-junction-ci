	<div class="container">
				<div class="row">
					<div class="col-md-12">
						...
						if(!is_array($applay_seeker))
						{
							...
							<div class="jp_job_box">
								...
								<div class="jp_no_doctor">
									...
									<p><?= $controller->set_language('applied_seeker_not_found'); ?></p>
								</div>
							<!-- no job found end -->
								</div>
							<!-- no job found end -->
						...
						{
						foreach($applay_seeker as $aseeler)
						{
							$seeker_data=$this->Common_model->select('seeker',$aseeler->s_email,'email');
						
						?>	
						<div class="jp_job_box">
							<div class="job_detail">
								<h3><?= $seeker_data->name ?></h3>
								<ul>
									<li><span><img src="<?= base_url('assets/images/experience.svg'); ?>" alt=""></span> <?= $seeker_data->exp; ?></li>
									<li><span><img src="<?= base_url('assets/images/location.svg'); ?>" alt=""></span> <?= $seeker_data->p_locaion ?></li>
								</ul>
							</div>
							<div class="jp_view_btn">
								<a href="<?= base_url('recruiter/seeker_informaion/'.$seeker_data->id.'/'.$aseeler->job_id); ?>" class="jp_btn jp_btn_block"><?= $controller->set_language('view_btn'); ?></a><br><br>
							</div>
							<div class="jp_view_btn">
								<a href="<?= base_url('recruiter/applied_job_information/'.$aseeler->job_id); ?>" class="jp_btn jp_btn_block"><?= $controller->set_language('job_btn'); ?></a>
							</div>
						</div>
						<?php
						}
